...
- 훅들을 같은 `hooks` 배열에 나열하면 순서대로 실행되고, **어느 하나라도** block을 반환하면 도구 호출이 차단된다.
  `ref-gate`를 맨 앞에 둔다 — 안 보고 만든 것이 가장 먼저 걸려야 나머지 판정이 의미를 갖는다.
...
| 필드 | 타입 | 값 | 의미 |
|---|---|---|---|
| `phase` | string | `"design"` \| `"sample"` \| `"bulk"` \| `"loop"` | 현재 단계. design=명세, sample=샘플 제작, bulk=전수 작업, loop=스크린샷 개선 루프 |
...
### ref-gate.sh
- state 파일 없음 → 통과
- `current_target`이 비어 있음 → 통과 (레퍼런스 «수집» 자체를 막으면 안 된다)
- `.design-bounce/ref/<대상>/`에 이미지(png/jpg/jpeg/webp) 1장이 없거나
  `intent.md`가 없음 → **차단**
- 둘 다 있음 → 통과
